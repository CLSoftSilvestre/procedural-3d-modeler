...
describe('registry', () => {
  it('registers the expected number of nodes', () => {
    // 14 primitives + transform + array + mirror + displace + twist + taper + bend + boolean
    // + polygon + star + extrude + lathe + material + random + expression + time + component + output
    expect(allNodeDefs().length).toBe(32);
  });
});
